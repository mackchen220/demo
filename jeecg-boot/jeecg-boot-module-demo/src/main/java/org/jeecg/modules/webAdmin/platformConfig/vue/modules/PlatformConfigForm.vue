        ...
        <a-row>
          ...
          <a-col :span="24">
            <a-form-item label="配置备注" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input v-decorator="['remark']" placeholder="请输入配置备注"  ></a-input>
            </a-form-item>
          </a-col>
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          add: "/platformConfig/platformConfig/add",
          edit: "/platformConfig/platformConfig/edit",
          queryById: "/platformConfig/platformConfig/queryById"
        }
      }
    },
    computed: {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'configValue','delFlag','remark'))
        })
      },
      //渲染流程表单数据
      showFlowData(){
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'configValue','delFlag','remark'))
      },
    }
  }
</script>