        ...
        <a-row>
          <a-col :span="24">
            <a-form-item label="接口地址" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input v-decorator="['sendApi', validatorRules.sendApi]" placeholder="请输入接口地址"  ></a-input>
            </a-form-item>
          </a-col>
          <a-col :span="24">
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          add: "/verified/adminVerifiedConfig/add",
          edit: "/verified/adminVerifiedConfig/edit",
          queryById: "/verified/adminVerifiedConfig/queryById"
        }
      }
    },
    computed: {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'sendApi','appId','appKey','delFlag'))
        })
      },
      //渲染流程表单数据
      showFlowData(){
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'sendApi','appId','appKey','delFlag'))
      },
    }
  }
</script>